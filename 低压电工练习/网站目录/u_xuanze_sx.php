...
?>
<?php 
	if(!is_numeric($_GET['id']) || !($_GET['id'] >= 1 && $_GET['id'] <= 402) ){
		header('Location:u_xuanze_sx.php?id=1');
		exit;
	}
	//echo $_GET['id'];
 ?>
...
$sql = "SELECT * FROM `xuanze` WHERE `id`='$sid'";
//echo $sql.'<br>';
...
<?php 
echo <<<EOF
<div class="info"><div class="num">第{$_GET['id']}题&nbsp;&nbsp;单选题</div><!-- <a href="javascript:void(0);" id="tag_24370" class="tag no" onclick="mytag(22,24370)">标记</a> --></div>

<div class="title" style="font-size: 1.2em;">{$title}</div>

<div class="row"><div class="relative"><input type="radio" onclick="squa('radio','{$_GET['id']}','A')" id="qs_A" name="qs" value="A"><i></i></div><label for="qs_A" style="font-size: 1.1em;">A. {$arr1[0]}</label></div>
<div class="row"><div class="relative"><input type="radio" onclick="squa('radio','{$_GET['id']}','B')" id="qs_B" name="qs" value="B"><i></i></div><label for="qs_B" style="font-size: 1.1em;">B. {$arr1[1]}</label></div>
<div class="row"><div class="relative"><input type="radio" onclick="squa('radio','{$_GET['id']}','C')" id="qs_C" name="qs" value="C"><i></i></div><label for="qs_C" style="font-size: 1.1em;">C. {$arr1[2]}</label></div>
<div class="row"><div class="relative"><input type="radio" onclick="squa('radio','{$_GET['id']}','D')" id="qs_D" name="qs" value="D"><i></i></div><label for="qs_D" style="font-size: 1.1em;">D. {$arr1[3]}</label></div>
<div id="warning"></div>
